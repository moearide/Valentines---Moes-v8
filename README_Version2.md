...
- GitHub Pages (if Pages is enabled for this repo):  
  https://moearide.github.io/Valentines---Moes-v8/

- HTML preview (renders the raw `index.html` directly):  
  https://htmlpreview.github.io/?https://raw.githubusercontent.com/moearide/Valentines---Moes-v8/main/index.html

- Raw OG image URL (use this for social meta or to replace the placeholder):  
  https://raw.githubusercontent.com/moearide/Valentines---Moes-v8/main/og-card.jpg

Notes:
- If the GitHub Pages URL returns 404, enable Pages in your repository Settings → Pages and choose branch `main` (or whichever branch you use) and the root (`/`) or `docs/` directory where `index.html` lives. GitHub may take a minute to publish.
- The HTML preview link uses the `htmlpreview` service and will render the current `index.html` from `main`. It's handy for quick sharing during editing.
...
```html
<meta property="og:image" content="https://raw.githubusercontent.com/moearide/Valentines---Moes-v8/main/og-card.jpg" />
```

Steps:
1. Create an OG image (recommended: 1200×630px, JPG/PNG).
2. Add it to your repo (e.g. `og-card.jpg` in repository root or `assets/`).
3. Update the `og:image` meta tag in `index.html` to point to the raw GitHub URL above.
